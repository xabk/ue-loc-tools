...
xabk mods for Unreal Exports to not split occurences on whitespace
and split and merge them with ', ' instead
Modified: line 1020 ' ' → ', '
Modified: line 1539 handle_oc ' ' → ', '
"""
...
class POEntry(_BaseEntry):
    ...
    def __unicode__(self, wrapwidth=78):
        ...
        if not self.obsolete and self.occurrences:
            ...
            filestr = ', '.join(filelist)  # MODIFIED
...
class _POFileParser(object):
    ...
    def handle_oc(self):
        ...
        if self.current_state in ['mc', 'ms', 'mx']:
            self.instance.append(self.current_entry)
            self.current_entry = POEntry(linenum=self.current_line)
        occurrences = self.current_token[3:].split(', ')  # MODIFIED TO ONLY SPLIT ON ', '
